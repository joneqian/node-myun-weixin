...
var api = {
  ...
  request: function (config, url, data, sendConfig, receiveConfig, cb) {
    ...
    params = this.prepare(config.app, config.merchant, params);
    var sign = this.sign(config.merchant, params);

    params.sign = sign;
    var xml = util.toXml(params);
    request.xmlssl(url, xml, config.certificate, function (error, json) {
      this.handle(config.app, config.merchant, json, receiveConfig, cb);
    });
  },
