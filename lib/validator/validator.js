...
var typeExt = ['enum', 'object', 'text', 'string', 'bool', 'boolean'];
...
var validate = {
  validateOne: function validateOne(data, conf) {
    ...
    for (var k in confs) {
      if (typeof k === 'string') {
        ...
        if (conf.required) {
          ...
          if (typeof param === 'string' && (validator.isNull(param) || !validator.isLength(param, 1))) {
            error.message = 'Key ' + k + ' is NULL';
            return error;
          }
        }
